...
export default function VideoSection({
  ...
  function loadYouTubeApiAndInit(idx: number) {
    activeIdxRef.current = idx;
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && !hasStartedRef.current) {
            handleStart(activeIdxRef.current);
          }
        });
      },
      { threshold: 0.3 },
    ...
    <>
      ...
      <section className="bg-[#FAF8F0] px-4 md:px-[40px] lg:px-[79px] md:pt-16  md:pb-20 pb-12">
        <div className="max-w-7xl mx-auto">
          ...
          <div
            ref={sectionRef}
            className="w-full rounded-[20px] overflow-hidden border-[6px] border-[#822C89] box-border flex flex-col md:grid md:grid-cols-[1fr_415px] md:h-[645px]"
          >
            <div className="relative bg-black h-[300px] md:h-full overflow-hidden">
              {/* YouTube player mounts here once started */}
              <div id="yt-player" className="absolute inset-0 w-full h-full" />

              {/* Facade — shown until user clicks play, zero YouTube JS loaded */}
              {!hasStarted && (
                <button
                  onClick={() => handleStart(activeIdx)}
                  aria-label={`Play video: ${videos[activeIdx].label}`}
                  className="absolute inset-0 w-full h-full cursor-pointer group"
                >
                  <Image
                    src={videos[activeIdx].image}
                    alt={videos[activeIdx].label}
                    fill
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="w-[64px] h-[64px] rounded-full bg-[#822C89] flex items-center justify-center shadow-[0_8px_24px_rgba(130,44,137,0.5)] group-hover:scale-110 transition-transform duration-300">
